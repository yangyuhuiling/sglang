...
At the layer, on an idle box, the bf16 wire does not clear the baseline's own
per-bucket variation and only pays at m_pad 16384; the fp8 wire pays from
m_pad 8192 and is worth -7.4% to -28.8% there. The floors in
``mori_gemm_ar.py`` encode that, so **the bf16 variant is expected to be close
to base here**, and a wash is the result rather than a failure.
